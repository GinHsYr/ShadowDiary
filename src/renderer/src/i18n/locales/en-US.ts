const enUS = {
  common: {
    appName: 'ShadowDiary',
    cancel: 'Cancel',
    save: 'Save',
    reset: 'Reset',
    done: 'Done',
    confirm: 'Confirm',
    close: 'Close',
    delete: 'Delete',
    loading: 'Loading...',
    noTitle: 'Untitled',
    minutes: '{count} min'
  },
  titleBar: {
    minimize: 'Minimize',
    maximize: 'Maximize',
    restore: 'Restore',
    close: 'Close'
  },
  app: {
    privacy: {
      title: 'Privacy Protection',
      unlockWithWindowsHello: 'Verify with Windows Hello to unlock',
      unlockWithPin: 'Enter your 6-digit PIN to unlock',
      unlock: 'Unlock',
      windowsHelloAction: 'Use Windows Hello',
      windowsHelloPrompt: 'Verify with Windows Hello to unlock ShadowDiary',
      windowsHelloFailed: 'Windows Hello verification failed, please try again',
      requirePin: 'Please enter a 6-digit PIN',
      pinIncorrect: 'Incorrect password, please try again',
      unlockFailed: 'Failed to unlock, please try again later'
    }
  },
  sidebar: {
    menu: {
      dashboard: 'Dashboard',
      today: 'Today',
      archives: 'Archives',
      media: 'Media Library',
      settings: 'Settings'
    },
    todayDiary: 'Write Today',
    editProfile: 'Edit Profile',
    uploadImage: 'Upload Image',
    resetAvatar: 'Reset',
    nickname: 'Nickname',
    nicknamePlaceholder: 'Enter nickname',
    defaultNickname: 'User',
    saveSuccess: 'Saved successfully',
    saveFailed: 'Failed to save, please try again',
    selectAvatarFailed: 'Failed to choose avatar, please try again'
  },
  header: {
    searchPlaceholder: 'Search memories... (Ctrl+K)',
    advancedFilter: 'Advanced Filters',
    mood: 'Mood',
    tags: 'Tags',
    date: 'Date',
    selectMood: 'Select mood',
    selectTags: 'Select tags',
    searching: 'Searching...',
    history: 'Search History',
    clear: 'Clear',
    archives: 'Archives',
    diary: 'Diary',
    person: 'Person',
    object: 'Object',
    other: 'Other',
    moreArchives: '{count} more archives',
    foundDiaryCount: '{count} related diary entries found',
    showTopCount: '(showing top {count})',
    emptyResult: 'No results found. Try another keyword?',
    shortcutNavigate: 'Navigate',
    shortcutSelect: 'Select',
    shortcutClose: 'Close',
    moodOptions: {
      happy: '😊 Happy',
      calm: '😌 Calm',
      sad: '😢 Sad',
      excited: '🤩 Excited',
      tired: '😴 Tired'
    }
  },
  archiveList: {
    searchPlaceholder: 'Search archives...',
    empty: 'No archives yet',
    create: '+ New Archive',
    draft: 'Draft',
    untitledDraft: 'Untitled archive',
    all: 'All',
    person: 'Person',
    object: 'Object',
    other: 'Other'
  },
  archiveDetail: {
    uploadAvatar: 'Click to upload avatar',
    name: 'Name',
    namePlaceholder: 'Enter name',
    untitled: 'Untitled archive',
    editName: 'Edit name',
    aliases: 'Aliases',
    aliasesHint: 'Press Enter to add multiple aliases',
    aliasPlaceholder: 'Add alias',
    type: 'Type',
    person: 'Person',
    object: 'Object',
    other: 'Other',
    description: 'About',
    descriptionTitle: 'Edit description',
    descriptionPlaceholder: 'Enter details...',
    descriptionEmpty: 'Click to add description...',
    editDescription: 'Click to edit description',
    images: 'Gallery',
    addImage: 'Add image',
    galleryEmpty: 'Click to add an image',
    imageCount: '{count} images',
    deleteArchive: 'Delete Archive',
    deleteConfirm: 'Are you sure you want to delete this archive?',
    saving: 'Saving...',
    unsaved: 'Unsaved',
    saved: 'Saved'
  },
  diaryList: {
    empty: 'No diaries yet',
    create: '+ New Diary',
    draft: 'Draft',
    selectDate: 'Select Date',
    confirm: 'Confirm',
    delete: 'Delete',
    locateSelected: 'Locate selected diary',
    quickScroll: 'Quick-scroll diary list',
    weekdayShort: ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'],
    dateFormat: '{month}/{day} {weekday}'
  },
  diaryEditor: {
    cut: 'Cut',
    copy: 'Copy',
    paste: 'Paste',
    bold: 'Bold',
    italic: 'Italic',
    underline: 'Underline',
    strikethrough: 'Strikethrough',
    clearFormatting: 'Clear Formatting',
    unsupportedImageType: 'Unsupported image format',
    imageTooLargeTitle: 'Image too large',
    imageTooLargeContent: 'Image "{name}" exceeds 10MB. Compress it and try again.',
    acknowledge: 'OK',
    placeholder: 'Write something...',
    searchOpen: 'Search in diary',
    searchPlaceholder: 'Search in this diary...',
    searchNoResult: 'No result',
    searchPrevious: 'Previous match',
    searchNext: 'Next match',
    searchClose: 'Close search',
    replacePlaceholder: 'Replace with...',
    replaceOne: 'Replace',
    replaceAll: 'All',
    characterCount: '{count} characters',
    paragraph: 'Paragraph',
    heading1: 'Heading 1',
    heading2: 'Heading 2',
    heading3: 'Heading 3',
    heading4: 'Heading 4'
  },
  today: {
    titlePlaceholder: 'Untitled',
    mood: {
      happy: 'Happy',
      calm: 'Calm',
      sad: 'Sad',
      excited: 'Excited',
      tired: 'Tired'
    },
    weekdays: ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'],
    currentDate: '{weekday}, {month}/{day}/{year}',
    deleteConfirmTitle: 'Delete Confirmation',
    deleteConfirmContent: 'Delete "{title}"?',
    deletePositive: 'Delete',
    tipTitle: 'Notice',
    missingDiary: 'Diary does not exist or has been deleted'
  },
  archivesPage: {
    empty: 'Select or create an archive'
  },
  imageLightbox: {
    counter: '{current} / {total}',
    prev: 'Previous',
    next: 'Next',
    close: 'Close'
  },
  mediaPage: {
    title: 'Media Library',
    subtitle: 'Browse all images from diaries and archives',
    dateFormat: '{month}/{day}/{year}',
    totalImages: '{count} images',
    filterAll: 'All',
    filterDiary: 'Diary',
    filterArchive: 'Archive',
    sourceDiary: 'Diary',
    sourceArchive: 'Archive',
    untitledArchive: 'Untitled archive',
    openSource: 'Open source',
    moreSources: '{count} more sources',
    empty: 'No images yet',
    emptyHint: 'Images inserted in diary editor or archive detail will appear here',
    loadingMore: 'Loading more images...',
    viewerIndex: '{current} / {total}',
    prevImage: 'Previous',
    nextImage: 'Next',
    closeViewer: 'Close'
  },
  dashboard: {
    welcome: '👋 Hi, ready to write something?',
    other: 'Other',
    mentionChartTitle: 'Person Mention Frequency',
    mentionTooltip: '{name}: {count} times ({percent}%)',
    moodChartTitle: 'Mood Trends',
    moodRange: {
      days7: '7D',
      days30: '30D',
      days90: '90D'
    },
    moodLevels: ['😢 Sad', '😴 Tired', '😌 Calm', '🤩 Excited', '😊 Happy'],
    moodEmpty: 'No mood records in this period',
    monthLabel: '{month}/{year}',
    yearLabel: '{year}',
    weekDays: ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'],
    months: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'],
    backToToday: 'Back to Today',
    quickJump: {
      today: 'Today',
      yesterday: 'Yesterday',
      lastWeek: 'Same day last week',
      lastMonth: 'Same day last month'
    },
    legend: {
      hasDiary: 'Has diary',
      today: 'Today'
    },
    stats: {
      totalEntriesLabel: 'You have written',
      totalEntriesSuffix: 'diaries',
      streakLabel: 'Current streak',
      streakSuffix: 'days',
      charsLabel: 'Total',
      charsSuffix: 'characters'
    },
    monthProgressTitle: 'This Month Progress',
    monthProgressMeta: '{written} / {total} days written',
    mentionModalTitle: '{person} · Mention Details',
    mentionMeta: 'Mentioned in {count} diaries',
    mentionEmpty: 'No mention records yet',
    mentionCount: 'Mentioned {count} times',
    openDiary: 'Open Diary'
  },
  settings: {
    title: 'Settings',
    subtitle: 'Personalize your diary app',
    appearance: 'Appearance',
    darkMode: 'Dark Mode',
    darkModeDescription: 'Toggle dark mode',
    themeAccent: 'Theme Accent',
    themeAccentDarkHint:
      'Black accent is unavailable in dark mode; if currently black, it will switch to green automatically',
    themeAccentHint: 'Choose your favorite color',
    language: 'Language',
    languageDescription: 'Set app display language',
    followSystem: 'Follow System',
    simplifiedChinese: 'Simplified Chinese',
    english: 'English',
    accents: {
      green: 'Emerald Green',
      blue: 'Sky Blue',
      black: 'Classic Black',
      orange: 'Amber Orange',
      red: 'Coral Red',
      purple: 'Neon Purple',
      teal: 'Teal'
    },
    sync: {
      card: 'Connected Devices',
      title: 'LAN Sync',
      subtitle: 'Securely exchange diaries with your phone on the same network',
      description: 'End-to-end encrypted sync for diaries, archives, and images—without a cloud',
      open: 'Open Sync',
      back: 'Back to Settings',
      enable: 'Enable LAN sync',
      enableDescription: 'Paired phones can discover this computer while ShadowDiary is running',
      secure:
        'Diary text, metadata, and images use AES-256-GCM and are available only to paired devices.',
      running: 'Running',
      stopped: 'Stopped',
      statusDescription:
        'Once enabled, the phone app finds this computer automatically on the same Wi-Fi.',
      lastSync: 'Last synced: {time}',
      status: {
        disabled: 'Sync service is paused',
        discovering: 'Waiting for your phone',
        pairing: 'Verification code ready',
        connecting: 'Verifying device',
        syncing: 'Exchanging memories',
        conflicts: 'Conflicts need attention',
        completed: 'Sync complete',
        failed: 'Sync was interrupted'
      },
      pairNew: 'Pair a New Phone',
      pairDescription: 'Create a one-time code, then select this computer on your phone',
      createCode: 'Create Code',
      pairingCode: 'One-time verification code',
      expires: 'Expires in {seconds}s',
      pairHint: 'A code valid for two minutes will appear here',
      pairedDevices: 'Paired Devices',
      pairedDescription: 'These devices can sync automatically when they reappear',
      noPairedDevices: 'No phone has been paired yet',
      neverSynced: 'First sync has not completed',
      unpair: 'Unpair',
      unpairTitle: 'Unpair this device?',
      unpairDescription:
        'After unpairing “{name}”, a new verification code is required to reconnect.',
      conflicts: 'Pending Conflicts',
      conflictsDescription: '{count} items were edited on both devices',
      diary: 'Diary conflict',
      archive: 'Archive conflict',
      resolveConflict: 'Choose the version to keep',
      desktopVersion: 'This computer',
      phoneVersion: 'Phone',
      keepDesktop: 'Keep computer',
      keepPhone: 'Keep phone',
      keepBoth: 'Keep both',
      deleted: 'Deleted',
      confirmResolutionTitle: 'Save this conflict resolution?',
      confirmResolutionDescription:
        'This will apply “{choice}”. Once saved, the conflict is removed from the pending list and cannot be undone here.',
      confirmSave: 'Confirm save',
      saveConfirmationHint: 'Your selection is saved only after confirmation',
      diff: {
        changed: 'Modified',
        reviewLabel: 'Git diff between the computer and phone versions',
        title: 'Title',
        date: 'Date',
        mood: 'Mood',
        weather: 'Weather',
        tags: 'Tags',
        content: 'Content',
        images: 'Images',
        richText: 'Rich-text source',
        name: 'Name',
        aliases: 'Aliases',
        type: 'Archive type',
        description: 'Description',
        mainImage: 'Main image',
        gallery: 'Gallery',
        createdAt: 'Created at',
        updatedAt: 'Updated at',
        other: 'Other raw differences',
        deleted: 'This version was deleted',
        omitted: '… {count} lines omitted …'
      },
      errorTitle: 'LAN sync issue',
      errors: {
        authentication: 'Device authentication failed. Unpair it and try again.',
        pairing: 'The verification code is invalid, expired, or has too many attempts.',
        asset: 'An image failed integrity verification, so this batch was not applied.',
        port: 'The LAN port could not be opened. Check your firewall settings.',
        connection:
          'The connection ended unexpectedly. Confirm both devices are on the same network.'
      }
    },
    ai: {
      title: 'AI Settings',
      subtitle: 'Configure providers, models, feature bindings, and local MCP access',
      entry: {
        title: 'Models and Providers',
        description: 'Configure AI providers, models, and feature bindings'
      },
      provider: {
        card: 'Providers',
        detailCard: 'Provider Configuration',
        modelCount: '{count} models',
        empty: 'No providers yet',
        selectProviderHint: 'Select a provider first',
        name: 'Provider Name',
        namePlaceholder: 'Enter provider name',
        baseUrl: 'Base URL',
        baseUrlPlaceholder: 'For example: https://api.openai.com/v1',
        apiKey: 'API Key',
        apiKeyDescription: 'API keys are stored locally through secure settings',
        apiKeyPlaceholder: 'Enter API key',
        customNamePlaceholder: 'Enter custom provider name',
        defaultCustomName: 'Custom Provider',
        deleteConfirmTitle: 'Delete Provider',
        deleteConfirmContent: 'Delete "{name}"? Its models and feature bindings will be removed.',
        types: {
          openai: 'OpenAI',
          anthropic: 'Anthropic',
          siliconflow: 'SiliconFlow',
          custom: 'Custom'
        }
      },
      model: {
        card: 'Models',
        empty: 'This provider has no models yet',
        displayNamePlaceholder: 'Display name, e.g. GPT-4o',
        modelIdPlaceholder: 'Model ID, e.g. gpt-4o'
      },
      binding: {
        card: 'Feature Model Bindings',
        providerPlaceholder: 'Select provider',
        modelPlaceholder: 'Select model'
      },
      mcp: {
        card: 'MCP Configuration',
        enabled: 'Enable MCP',
        enabledDescription: 'Allow other local apps to read diaries through MCP',
        endpoint: 'Local Endpoint',
        endpointDescription: 'Listens on 127.0.0.1 only and requires a Bearer token',
        port: 'Port',
        portDescription: 'Changing the port restarts the MCP service automatically',
        token: 'Access Token',
        tokenDescription: 'Send this as Authorization: Bearer when calling MCP',
        maxSearchResults: 'Max Search Results',
        maxReadChars: 'Read Characters',
        maxBatchMetadata: 'Batch Metadata Limit',
        statusRunning: 'Running',
        statusStopped: 'Stopped',
        statusStarting: 'Starting',
        statusError: 'Error',
        statusErrorDetail: 'MCP failed to start: {reason}',
        openDetail: 'Open Configuration',
        usageTitle: 'Usage Guide',
        usageDescription: 'Add ShadowDiary to an MCP client that supports Streamable HTTP.',
        clientSetupTitle: 'Client Setup',
        clientSetupEnable: 'Enable MCP and confirm the local endpoint is running.',
        clientSetupCopy:
          'Copy the client configuration below into the MCP client mcpServers config.',
        clientSetupRestart: 'After saving the client configuration, reload or restart that client.',
        authTitle: 'Authentication',
        authDescription:
          'Every request must include the access token. Update the client config after changing the token.',
        clientConfigTitle: 'Client Config Example',
        toolsTitle: 'Available Tools',
        toolDiarySearch:
          'Search diaries by keyword, date, mood, or tags. Returns snippets and entry IDs.',
        toolDiaryRead: 'Read one diary as paginated plain text by ID.',
        toolDiaryReadByDate: 'Read diary plain text by date or date range.',
        toolDiaryGetMetadataBatch:
          'Read metadata for one or more diaries by ID without returning content.',
        toolArchiveSearchByName:
          'Search archive content by name, alias, or any partial keyword. Names and aliases are matched without distinction.'
      },
      features: {
        chat: 'Chat',
        summary: 'Summary',
        rewrite: 'Rewrite'
      },
      actions: {
        addProvider: 'Add Provider',
        deleteProvider: 'Delete Provider',
        addModel: 'Add Model',
        deleteModel: 'Delete Model',
        clearBinding: 'Clear',
        copyToken: 'Copy Token',
        copyConfig: 'Copy Config',
        regenerateToken: 'Regenerate',
        openDetail: 'Open Settings'
      },
      messages: {
        saving: 'Saving...',
        saveSuccess: 'AI settings saved',
        saveFailed: 'Failed to save AI settings',
        providerAdded: 'Provider added',
        providerDeleted: 'Provider deleted',
        modelAdded: 'Model added',
        modelDeleted: 'Model deleted',
        modelAddFailed: 'Failed to add model',
        modelUpdateFailed: 'Failed to update model',
        providerHasNoModel: 'This provider has no models yet',
        tokenCopied: 'MCP token copied',
        tokenCopyFailed: 'Failed to copy MCP token',
        tokenRegenerated: 'MCP token regenerated',
        configCopied: 'MCP client config copied',
        configCopyFailed: 'Failed to copy MCP client config'
      }
    },
    privacy: {
      card: 'Privacy Protection',
      enabled: 'Privacy Protection',
      enabledDescription: 'When enabled, unlocking credentials are required after re-entering',
      authMethod: 'Unlock Method',
      authMethodSupported: 'Choose a 6-digit PIN or Windows Hello',
      authMethodPinOnly: 'Only the 6-digit PIN unlock method is available on this device',
      pinOption: '6-digit PIN',
      windowsOption: 'Windows Hello',
      idleLock: 'Idle Auto-Lock',
      idleLockDescription: 'Lock automatically after the configured idle duration',
      passwordSetup: 'Password Setup',
      passwordSetupHint: 'Reset password in the modal (current password required)',
      passwordMissingHint: 'No password is set. Please set a 6-digit PIN first',
      resetPassword: 'Reset Password',
      setPassword: 'Set Password',
      windowsHelloLabel: 'Windows Hello',
      windowsHelloDescription:
        'Uses your Windows Hello verification flow, including PIN or available biometrics',
      manualLockShortcut: 'Manual Lock Shortcut',
      manualLockShortcutDescription:
        'When privacy protection is enabled, use this shortcut to lock immediately (focus the input then press a new combo)',
      disguiseEnabled: 'Disguise Mode',
      disguiseEnabledDescription:
        'Switch to editable fake data across the app. Turn off to return to real data',
      disguiseAutoEnableOnLaunch: 'Auto-enter disguise mode on launch',
      disguiseAutoEnableOnLaunchDescription:
        'When enabled, app launch will auto-apply disguise mode based on last state',
      disguiseShortcut: 'Disguise Mode Shortcut',
      disguiseShortcutDescription: 'Use this shortcut to quickly toggle disguise mode',
      resetDisguiseShortcut: 'Reset Default',
      regenerateDisguiseData: 'Regenerate Disguise Data',
      regenerateDisguiseDataDescription: 'Generate a new fake dataset (only affects disguise mode)',
      regenerateDisguiseDataAction: 'Regenerate',
      enabledSuccess: 'Privacy protection enabled',
      disabledSuccess: 'Privacy protection disabled',
      disguiseEnabledSuccess: 'Disguise mode enabled, reloading page',
      disguiseDisabledSuccess: 'Disguise mode disabled, restoring real data',
      disguiseAutoEnableUpdated: 'Disguise mode launch behavior updated',
      disguiseShortcutUpdated: 'Disguise mode shortcut updated to {shortcut}',
      disguiseShortcutInvalid: 'Invalid shortcut. Use Ctrl/Alt/Shift/Meta + letter or number',
      disguiseShortcutConflict: 'Disguise mode shortcut cannot be the same as manual lock shortcut',
      disguiseDataRegenerated: 'Disguise data regenerated, reloading page',
      switchToWindows: 'Switched to Windows Hello unlock',
      switchToPin: 'Switched to PIN unlock',
      windowsHelloEnablePrompt: 'Verify with Windows Hello to enable privacy protection',
      windowsHelloDisablePrompt: 'Verify with Windows Hello to disable privacy protection',
      windowsHelloVerifyFailed: 'Windows Hello verification was not completed',
      idleLockUpdated: 'Idle lock duration updated',
      manualLockShortcutUpdated: 'Manual lock shortcut updated to {shortcut}',
      manualLockShortcutInvalid: 'Invalid shortcut. Use Ctrl/Alt/Shift/Meta + letter or number',
      resetManualLockShortcut: 'Reset Default',
      passwordResetSuccess: 'Password reset successfully',
      passwordSetAndEnabled: 'Password set successfully, privacy protection enabled',
      passwordSetAndSwitched: 'Password set successfully, switched to PIN unlock',
      passwordSetSuccess: 'Password set successfully',
      modal: {
        setupTitle: 'Set Privacy Password',
        disableTitle: 'Disable Privacy Protection',
        resetTitle: 'Reset Privacy Password',
        setupDesc: 'Please set a 6-digit PIN',
        setupSwitchDesc: 'Set a 6-digit PIN first. Unlock method will switch to PIN after saving',
        disableDesc: 'Enter current password to disable privacy protection',
        resetDesc: 'Verify current password before setting a new one',
        currentPassword: 'Current Password',
        newPassword: 'New Password',
        confirmPassword: 'Confirm New Password',
        confirmDisable: 'Confirm Disable'
      }
    },
    data: {
      card: 'Data Management',
      export: 'Export Data',
      exportDescription:
        'Export database and attachments to a ZIP backup. Image directory is packed into a password-protected ZIP',
      import: 'Import Data',
      importDescription:
        'Restore data from ZIP backup. This overwrites local data and requires the backup password',
      backupPassword: 'Backup Password',
      confirmPassword: 'Confirm Password',
      minLength: 'At least {count} characters',
      reenterPassword: 'Re-enter password',
      exportPasswordTitle: 'Export Backup Password',
      importPasswordTitle: 'Import Backup Password',
      exportPasswordDesc:
        'Set a password for this backup. It is required when importing this backup',
      importPasswordDesc: 'Enter the backup password that was set during export',
      confirmExport: 'Confirm Export',
      confirmImport: 'Confirm Import',
      exportInProgress: 'Exporting backup',
      importInProgress: 'Importing backup',
      preparingExport: 'Preparing export task...',
      preparingImport: 'Preparing import task...',
      exporting: 'Exporting data...',
      importing: 'Importing data...',
      cancelingExport: 'Canceling export...',
      cancelingImport: 'Canceling import...',
      exportCanceled: 'Export canceled',
      importCanceled: 'Import canceled',
      canceledExportMessage: 'Export canceled',
      canceledImportMessage: 'Import canceled',
      exportCompleted: 'Export completed',
      importCompleted: 'Import completed',
      exportFailed: 'Export failed',
      importFailed: 'Import failed',
      importSuccessAndReload: 'Import succeeded. Click Done to refresh the page',
      exportSuccessWithPath: 'Export succeeded: {path}',
      exportAction: 'Export',
      importAction: 'Import',
      failedWithReason: '{action} failed: {reason}',
      updateFailedWithReason: 'Update failed: {reason}',
      openFailedWithReason: 'Enable failed: {reason}',
      closeFailedWithReason: 'Disable failed: {reason}',
      saveFailedWithReason: 'Save failed: {reason}',
      checkUpdateFailedWithReason: 'Failed to check updates: {reason}',
      downloadUpdateFailedWithReason: 'Failed to download update: {reason}',
      unknownError: 'Unknown error',
      wrongBackupPassword: 'Incorrect backup password, please try again',
      backupPasswordTooShort: 'Backup password must be at least {count} characters',
      unsupportedBackup:
        'Unsupported backup format. Only backups generated by the current app version are supported',
      missingEnvelope: 'Backup is missing key file and cannot be imported',
      transferInProgress: 'Another import/export task is already running',
      importConfirmTitle: 'Import Confirmation',
      importConfirmContent:
        'Importing a ZIP backup will overwrite local data. It is recommended to export a backup first. Continue?',
      continueImport: 'Continue Import'
    },
    about: {
      card: 'About',
      version: 'Version',
      source: 'Source',
      checkUpdate: 'Check for new version',
      checkUpdateButton: 'Check Updates',
      downloadUpdate: 'Download Update',
      installNow: 'Install Now',
      updateFound: 'New version found: {version}',
      latest: 'You are using the latest version',
      downloading: 'Downloading update...',
      downloadProgressDetail: 'Downloaded {transferred} / Total {total}',
      cancelingDownload: 'Canceling download...',
      downloadCanceled: 'Update download canceled',
      downloaded: 'Update downloaded. Click Install to restart and apply updates'
    }
  }
}

export default enUS
