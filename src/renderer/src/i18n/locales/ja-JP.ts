const jaJP = {
  common: {
    appName: 'ShadowDiary',
    cancel: 'キャンセル',
    save: '保存',
    reset: 'リセット',
    done: '完了',
    confirm: '確認',
    close: '閉じる',
    delete: '削除',
    loading: '読み込み中...',
    noTitle: '無題',
    minutes: '{count} 分'
  },
  titleBar: {
    minimize: '最小化',
    maximize: '最大化',
    restore: '元に戻す',
    close: '閉じる'
  },
  app: {
    privacy: {
      title: 'プライバシー保護',
      unlockWithWindowsPassword: 'Windows サインインパスワードを入力してロックを解除してください',
      unlockWithPin: '6 桁の PIN を入力してロックを解除してください',
      windowsPasswordPlaceholder: 'Windows サインインパスワード',
      unlock: 'ロック解除',
      requireWindowsPassword: 'Windows サインインパスワードを入力してください',
      requirePin: '6 桁の PIN を入力してください',
      windowsPasswordIncorrect: 'Windows パスワードが正しくありません。もう一度お試しください',
      pinIncorrect: 'パスワードが正しくありません。もう一度お試しください',
      unlockFailed: 'ロック解除に失敗しました。しばらくしてから再試行してください'
    }
  },
  sidebar: {
    menu: {
      dashboard: 'ダッシュボード',
      today: '今日',
      archives: 'アーカイブ',
      media: 'メディアライブラリ',
      settings: '設定'
    },
    todayDiary: '今日の日記を書く',
    editProfile: 'プロフィールを編集',
    uploadImage: '画像をアップロード',
    resetAvatar: 'リセット',
    nickname: 'ニックネーム',
    nicknamePlaceholder: 'ニックネームを入力',
    defaultNickname: 'ユーザー',
    saveSuccess: '保存しました',
    saveFailed: '保存に失敗しました。もう一度お試しください',
    selectAvatarFailed: '画像の選択に失敗しました。もう一度お試しください'
  },
  header: {
    searchPlaceholder: '思い出を検索... (Ctrl+K)',
    advancedFilter: '詳細フィルター',
    mood: '気分',
    tags: 'タグ',
    date: '日付',
    selectMood: '気分を選択',
    selectTags: 'タグを選択',
    searching: '検索中...',
    history: '検索履歴',
    clear: 'クリア',
    archives: 'アーカイブ',
    diary: '日記',
    person: '人物',
    object: '物',
    other: 'その他',
    moreArchives: 'さらに {count} 件のアーカイブ',
    foundDiaryCount: '関連する日記が {count} 件見つかりました',
    showTopCount: '(上位 {count} 件を表示)',
    emptyResult: '結果が見つかりません。別のキーワードをお試しください。',
    shortcutNavigate: '移動',
    shortcutSelect: '選択',
    shortcutClose: '閉じる',
    moodOptions: {
      happy: '😊 うれしい',
      calm: '😌 落ち着く',
      sad: '😢 悲しい',
      excited: '🤩 ワクワク',
      tired: '😴 疲れた'
    }
  },
  archiveList: {
    searchPlaceholder: 'アーカイブを検索...',
    empty: 'アーカイブはまだありません',
    create: '+ 新規アーカイブ',
    draft: '下書き',
    untitledDraft: '無題のアーカイブ',
    all: 'すべて',
    person: '人物',
    object: '物',
    other: 'その他'
  },
  archiveDetail: {
    uploadAvatar: 'クリックしてアバターをアップロード',
    name: '名前',
    namePlaceholder: '名前を入力',
    untitled: '無題のアーカイブ',
    editName: '名前を編集',
    aliases: '別名',
    aliasesHint: 'Enter キーで複数の別名を追加',
    aliasPlaceholder: '別名を追加',
    type: '種類',
    person: '人物',
    object: '物',
    other: 'その他',
    description: '紹介',
    descriptionTitle: '紹介を編集',
    descriptionPlaceholder: '詳細を入力...',
    descriptionEmpty: 'クリックして紹介を追加...',
    editDescription: 'クリックして紹介を編集',
    images: 'ギャラリー',
    addImage: '画像を追加',
    galleryEmpty: 'クリックして画像を追加',
    imageCount: '{count} 枚',
    deleteArchive: 'アーカイブを削除',
    deleteConfirm: 'このアーカイブを削除してもよろしいですか？',
    saving: '保存中...',
    unsaved: '未保存',
    saved: '保存済み'
  },
  diaryList: {
    empty: '日記はまだありません',
    create: '+ 新規日記',
    draft: '下書き',
    selectDate: '日付を選択',
    confirm: '確認',
    delete: '削除',
    locateSelected: '選択中の日記へ移動',
    quickScroll: '日記リストをすばやくスクロール',
    weekdayShort: ['日', '月', '火', '水', '木', '金', '土'],
    dateFormat: '{month}/{day} ({weekday})'
  },
  diaryEditor: {
    cut: '切り取り',
    copy: 'コピー',
    paste: '貼り付け',
    bold: '太字',
    italic: '斜体',
    underline: '下線',
    strikethrough: '取り消し線',
    clearFormatting: '書式をクリア',
    unsupportedImageType: 'サポートされていない画像形式です',
    imageTooLargeTitle: '画像サイズが大きすぎます',
    imageTooLargeContent: '画像「{name}」は 10MB を超えています。圧縮してから再試行してください。',
    acknowledge: 'OK',
    placeholder: '何か書いてみましょう...',
    searchOpen: '日記内を検索',
    searchPlaceholder: 'この日記内を検索...',
    searchNoResult: '該当なし',
    searchPrevious: '前の一致',
    searchNext: '次の一致',
    searchClose: '検索を閉じる',
    replacePlaceholder: '置換後の文字列...',
    replaceOne: '置換',
    replaceAll: 'すべて',
    characterCount: '{count} 文字',
    paragraph: '本文',
    heading1: '見出し 1',
    heading2: '見出し 2',
    heading3: '見出し 3',
    heading4: '見出し 4'
  },
  today: {
    titlePlaceholder: '無題',
    mood: {
      happy: 'うれしい',
      calm: '落ち着く',
      sad: '悲しい',
      excited: 'ワクワク',
      tired: '疲れた'
    },
    weekdays: ['日曜日', '月曜日', '火曜日', '水曜日', '木曜日', '金曜日', '土曜日'],
    currentDate: '{year}/{month}/{day} ({weekday})',
    deleteConfirmTitle: '削除の確認',
    deleteConfirmContent: '「{title}」を削除しますか？',
    deletePositive: '削除',
    tipTitle: 'お知らせ',
    missingDiary: '日記が存在しないか、すでに削除されています'
  },
  archivesPage: {
    empty: 'アーカイブを選択または作成してください'
  },
  imageLightbox: {
    counter: '{current} / {total}',
    prev: '前へ',
    next: '次へ',
    close: '閉じる'
  },
  mediaPage: {
    title: 'メディアライブラリ',
    subtitle: '日記とアーカイブ内のすべての画像を表示します',
    dateFormat: '{year}年{month}月{day}日',
    totalImages: '{count} 枚の画像',
    filterAll: 'すべて',
    filterDiary: '日記',
    filterArchive: 'アーカイブ',
    sourceDiary: '日記',
    sourceArchive: 'アーカイブ',
    untitledArchive: '無題のアーカイブ',
    openSource: '元を開く',
    moreSources: '他 {count} 件',
    empty: '画像がありません',
    emptyHint: '日記エディタまたはアーカイブ詳細で追加した画像がここに表示されます',
    loadingMore: '画像を読み込み中...',
    viewerIndex: '{current} / {total} 枚目',
    prevImage: '前の画像',
    nextImage: '次の画像',
    closeViewer: '閉じる'
  },
  dashboard: {
    welcome: '👋 こんにちは、何か書いてみませんか？',
    other: 'その他',
    mentionChartTitle: '人物の言及頻度',
    mentionTooltip: '{name}: {count} 回 ({percent}%)',
    moodChartTitle: '気分の推移',
    moodRange: {
      days7: '7日',
      days30: '30日',
      days90: '90日'
    },
    moodLevels: ['😢 悲しい', '😴 疲れた', '😌 穏やか', '🤩 興奮', '😊 楽しい'],
    moodEmpty: 'この期間の気分記録はありません',
    monthLabel: '{year}/{month}',
    yearLabel: '{year}',
    weekDays: ['日', '月', '火', '水', '木', '金', '土'],
    months: ['1月', '2月', '3月', '4月', '5月', '6月', '7月', '8月', '9月', '10月', '11月', '12月'],
    backToToday: '今日に戻る',
    quickJump: {
      today: '今日',
      yesterday: '昨日',
      lastWeek: '先週の同じ曜日',
      lastMonth: '先月の同じ日'
    },
    legend: {
      hasDiary: '日記あり',
      today: '今日'
    },
    stats: {
      totalEntriesLabel: 'これまでに',
      totalEntriesSuffix: '件の日記を書きました',
      streakLabel: '連続記録',
      streakSuffix: '日',
      charsLabel: '合計',
      charsSuffix: '文字'
    },
    monthProgressTitle: '今月の進捗',
    monthProgressMeta: '{written} / {total} 日記録',
    mentionModalTitle: '{person} ・ 言及詳細',
    mentionMeta: '{count} 件の日記で言及',
    mentionEmpty: '言及記録はまだありません',
    mentionCount: '{count} 回言及',
    openDiary: '日記を開く'
  },
  settings: {
    title: '設定',
    subtitle: '日記アプリを自分好みにカスタマイズ',
    appearance: '表示',
    darkMode: 'ダークモード',
    darkModeDescription: 'ダークモードを切り替え',
    themeAccent: 'テーマカラー',
    themeAccentDarkHint:
      'ダークモードではブラックのテーマカラーは使用できません。現在ブラックの場合は自動でグリーンに切り替わります',
    themeAccentHint: 'お好みの色を選択してください',
    language: '言語',
    languageDescription: 'アプリの表示言語を設定',
    followSystem: 'システムに従う',
    simplifiedChinese: '简体中文',
    english: 'English',
    accents: {
      green: 'エメラルドグリーン',
      blue: 'スカイブルー',
      black: 'クラシックブラック',
      orange: 'アンバーオレンジ',
      red: 'コーラルレッド',
      purple: 'ネオンパープル',
      teal: 'ティール'
    },
    sync: {
      card: '接続デバイス',
      title: 'LAN 同期',
      subtitle: '同じネットワーク上のスマートフォンと日記を安全に同期します',
      description: 'クラウドを介さず、日記・アーカイブ・画像をエンドツーエンド暗号化で同期',
      open: '同期を開く',
      back: '設定に戻る',
      enable: 'LAN 同期を有効化',
      enableDescription: 'ShadowDiary の実行中、ペアリング済みの端末がこの PC を検出できます',
      secure:
        '本文、メタデータ、画像は AES-256-GCM で暗号化され、ペアリング済み端末のみアクセスできます。',
      running: '実行中',
      stopped: '停止中',
      statusDescription: '有効にすると、同じ Wi-Fi 上のスマートフォンがこの PC を自動検出します。',
      lastSync: '最終同期：{time}',
      status: {
        disabled: '同期サービスは一時停止中',
        discovering: 'スマートフォンを待機中',
        pairing: '確認コードを発行しました',
        connecting: 'デバイスを確認中',
        syncing: '記録を同期中',
        conflicts: '競合の確認が必要です',
        completed: '同期完了',
        failed: '同期が中断されました'
      },
      pairNew: '新しいスマートフォンをペアリング',
      pairDescription: 'ワンタイムコードを発行し、スマートフォンでこの PC を選択してください',
      createCode: 'コードを発行',
      pairingCode: 'ワンタイム確認コード',
      expires: 'あと {seconds} 秒で失効',
      pairHint: '2 分間有効なコードがここに表示されます',
      pairedDevices: 'ペアリング済みデバイス',
      pairedDescription: '再び検出されると自動で同期できます',
      noPairedDevices: 'ペアリング済みのスマートフォンはありません',
      neverSynced: '初回同期はまだ完了していません',
      unpair: 'ペアリング解除',
      unpairTitle: 'このデバイスとのペアリングを解除しますか？',
      unpairDescription: '「{name}」を解除すると、再接続に新しい確認コードが必要です。',
      conflicts: '未解決の競合',
      conflictsDescription: '{count} 件が両方のデバイスで編集されています',
      diary: '日記の競合',
      archive: 'アーカイブの競合',
      resolveConflict: '保持するバージョンを選択',
      desktopVersion: 'この PC',
      phoneVersion: 'スマートフォン',
      keepDesktop: 'PC 版を保持',
      keepPhone: 'スマートフォン版を保持',
      keepBoth: '両方を保持',
      deleted: '削除済み',
      confirmResolutionTitle: '競合の解決結果を保存しますか？',
      confirmResolutionDescription:
        '「{choice}」を適用します。保存すると、この競合は未解決リストから削除され、ここでは元に戻せません。',
      confirmSave: '保存を確定',
      saveConfirmationHint: 'バージョンを選択し、確認すると保存されます',
      diff: {
        changed: '変更あり',
        reviewLabel: 'PC 版とスマートフォン版の Git 差分',
        title: 'タイトル',
        date: '日付',
        mood: '気分',
        weather: '天気',
        tags: 'タグ',
        content: '本文',
        images: '画像',
        richText: 'リッチテキストソース',
        name: '名前',
        aliases: '別名',
        type: 'アーカイブ種類',
        description: '説明',
        mainImage: 'メイン画像',
        gallery: 'ギャラリー',
        createdAt: '作成日時',
        updatedAt: '更新日時',
        other: 'その他の元データ差分',
        deleted: 'このバージョンは削除済み',
        omitted: '… {count} 行を省略 …'
      },
      errorTitle: 'LAN 同期エラー',
      errors: {
        authentication: 'デバイス認証に失敗しました。ペアリングを解除して再試行してください。',
        pairing: '確認コードが無効、期限切れ、または試行回数を超えています。',
        asset: '画像の整合性確認に失敗したため、この同期は適用されませんでした。',
        port: 'LAN ポートを開けません。ファイアウォール設定を確認してください。',
        connection:
          '接続が中断されました。両方のデバイスが同じネットワークにあるか確認してください。'
      }
    },
    ai: {
      title: 'AI Settings',
      subtitle: 'Configure providers, models, feature bindings, and local MCP access',
      entry: {
        title: 'Models and Providers',
        description: 'Configure AI providers, models, and feature bindings'
      },
      provider: {
        card: 'Providers',
        detailCard: 'Provider Configuration',
        modelCount: '{count} models',
        empty: 'No providers yet',
        selectProviderHint: 'Select a provider first',
        name: 'Provider Name',
        namePlaceholder: 'Enter provider name',
        baseUrl: 'Base URL',
        baseUrlPlaceholder: 'For example: https://api.openai.com/v1',
        apiKey: 'API Key',
        apiKeyDescription: 'API keys are stored locally through secure settings',
        apiKeyPlaceholder: 'Enter API key',
        customNamePlaceholder: 'Enter custom provider name',
        defaultCustomName: 'Custom Provider',
        deleteConfirmTitle: 'Delete Provider',
        deleteConfirmContent: 'Delete "{name}"? Its models and feature bindings will be removed.',
        types: {
          openai: 'OpenAI',
          anthropic: 'Anthropic',
          siliconflow: 'SiliconFlow',
          custom: 'Custom'
        }
      },
      model: {
        card: 'Models',
        empty: 'This provider has no models yet',
        displayNamePlaceholder: 'Display name, e.g. GPT-4o',
        modelIdPlaceholder: 'Model ID, e.g. gpt-4o'
      },
      binding: {
        card: 'Feature Model Bindings',
        providerPlaceholder: 'Select provider',
        modelPlaceholder: 'Select model'
      },
      mcp: {
        card: 'MCP Configuration',
        enabled: 'Enable MCP',
        enabledDescription: 'Allow other local apps to read diaries through MCP',
        endpoint: 'Local Endpoint',
        endpointDescription: 'Listens on 127.0.0.1 only and requires a Bearer token',
        port: 'Port',
        portDescription: 'Changing the port restarts the MCP service automatically',
        token: 'Access Token',
        tokenDescription: 'Send this as Authorization: Bearer when calling MCP',
        maxSearchResults: 'Max Search Results',
        maxReadChars: 'Read Characters',
        maxBatchMetadata: 'Batch Metadata Limit',
        statusRunning: 'Running',
        statusStopped: 'Stopped',
        statusStarting: 'Starting',
        statusError: 'Error',
        statusErrorDetail: 'MCP failed to start: {reason}',
        openDetail: 'Open Configuration',
        usageTitle: 'Usage Guide',
        usageDescription: 'Add ShadowDiary to an MCP client that supports Streamable HTTP.',
        clientSetupTitle: 'Client Setup',
        clientSetupEnable: 'Enable MCP and confirm the local endpoint is running.',
        clientSetupCopy:
          'Copy the client configuration below into the MCP client mcpServers config.',
        clientSetupRestart: 'After saving the client configuration, reload or restart that client.',
        authTitle: 'Authentication',
        authDescription:
          'Every request must include the access token. Update the client config after changing the token.',
        clientConfigTitle: 'Client Config Example',
        toolsTitle: 'Available Tools',
        toolDiarySearch:
          'Search diaries by keyword, date, mood, or tags. Returns snippets and entry IDs.',
        toolDiaryRead: 'Read one diary as paginated plain text by ID.',
        toolDiaryReadByDate: 'Read diary plain text by date or date range.',
        toolDiaryGetMetadataBatch:
          'Read metadata for one or more diaries by ID without returning content.',
        toolArchiveSearchByName:
          '名前・別名・任意のキーワードでアーカイブ内容（人物・物など）を検索します。名前と別名を区別せずに自動的にマッチします。'
      },
      features: {
        chat: 'Chat',
        summary: 'Summary',
        rewrite: 'Rewrite'
      },
      actions: {
        addProvider: 'Add Provider',
        deleteProvider: 'Delete Provider',
        addModel: 'Add Model',
        deleteModel: 'Delete Model',
        clearBinding: 'Clear',
        copyToken: 'Copy Token',
        copyConfig: 'Copy Config',
        regenerateToken: 'Regenerate',
        openDetail: 'Open Settings'
      },
      messages: {
        saving: 'Saving...',
        saveSuccess: 'AI settings saved',
        saveFailed: 'Failed to save AI settings',
        providerAdded: 'Provider added',
        providerDeleted: 'Provider deleted',
        modelAdded: 'Model added',
        modelDeleted: 'Model deleted',
        modelAddFailed: 'Failed to add model',
        modelUpdateFailed: 'Failed to update model',
        providerHasNoModel: 'This provider has no models yet',
        tokenCopied: 'MCP token copied',
        tokenCopyFailed: 'Failed to copy MCP token',
        tokenRegenerated: 'MCP token regenerated',
        configCopied: 'MCP client config copied',
        configCopyFailed: 'Failed to copy MCP client config'
      }
    },
    privacy: {
      card: 'プライバシー保護',
      enabled: 'プライバシー保護',
      enabledDescription: '有効にすると、再入場時に解除用の認証情報が必要になります',
      authMethod: '解除方法',
      authMethodSupported: '6 桁 PIN または Windows サインインパスワードを選択できます',
      authMethodPinOnly: 'このシステムでは 6 桁 PIN のみサポートされています',
      pinOption: '6 桁 PIN',
      windowsOption: 'Windows サインインパスワード',
      idleLock: 'アイドル時自動ロック',
      idleLockDescription: '設定した無操作時間後に自動でロックします',
      passwordSetup: 'パスワード設定',
      passwordSetupHint: 'モーダルでパスワードを再設定（現在のパスワードが必要）',
      passwordMissingHint: 'パスワードが未設定です。先に 6 桁 PIN を設定してください',
      resetPassword: 'パスワード再設定',
      setPassword: 'パスワード設定',
      windowsPassword: 'Windows サインインパスワード',
      windowsPasswordDescription:
        '解除にはシステムアカウントのパスワードを使用します。ローカルのプライバシーパスワードは不要です',
      manualLockShortcut: '手動ロックショートカット',
      manualLockShortcutDescription:
        'プライバシー保護が有効なとき、このショートカットで即時ロックできます（入力欄を選択して新しい組み合わせを押してください）',
      disguiseEnabled: '偽装モード',
      disguiseEnabledDescription:
        '有効にすると、アプリ全体が編集可能なダミーデータ表示に切り替わります',
      disguiseAutoEnableOnLaunch: '起動時に自動で偽装モードへ',
      disguiseAutoEnableOnLaunchDescription:
        '有効時は前回状態に基づき、次回起動時に偽装モードを自動適用します',
      disguiseShortcut: '偽装モードショートカット',
      disguiseShortcutDescription: 'このショートカットで偽装モードを素早く切り替えます',
      resetDisguiseShortcut: 'デフォルトに戻す',
      regenerateDisguiseData: '偽装データを再生成',
      regenerateDisguiseDataDescription: '新しいダミーデータを再生成します（偽装モードのみ）',
      regenerateDisguiseDataAction: '再生成',
      enabledSuccess: 'プライバシー保護を有効にしました',
      disabledSuccess: 'プライバシー保護を無効にしました',
      disguiseEnabledSuccess: '偽装モードを有効にしました。ページを再読み込みします',
      disguiseDisabledSuccess: '偽装モードを無効にしました。実データへ戻します',
      disguiseAutoEnableUpdated: '偽装モードの起動動作を更新しました',
      disguiseShortcutUpdated: '偽装モードショートカットを {shortcut} に更新しました',
      disguiseShortcutInvalid:
        '無効なショートカットです。Ctrl/Alt/Shift/Meta + 英字または数字を使用してください',
      disguiseShortcutConflict:
        '偽装モードショートカットは手動ロックショートカットと同じにできません',
      disguiseDataRegenerated: '偽装データを再生成しました。ページを再読み込みします',
      switchToWindows: 'Windows パスワードでの解除に切り替えました',
      switchToPin: 'PIN での解除に切り替えました',
      idleLockUpdated: 'アイドルロック時間を更新しました',
      manualLockShortcutUpdated: '手動ロックショートカットを {shortcut} に更新しました',
      manualLockShortcutInvalid:
        '無効なショートカットです。Ctrl/Alt/Shift/Meta + 英字または数字を使用してください',
      resetManualLockShortcut: 'デフォルトに戻す',
      passwordResetSuccess: 'パスワードを再設定しました',
      passwordSetAndEnabled: 'パスワードを設定し、プライバシー保護を有効にしました',
      passwordSetAndSwitched: 'パスワードを設定し、PIN 解除に切り替えました',
      passwordSetSuccess: 'パスワードを設定しました',
      modal: {
        setupTitle: 'プライバシーパスワードを設定',
        disableTitle: 'プライバシー保護を無効化',
        resetTitle: 'プライバシーパスワードを再設定',
        setupDesc: '6 桁の PIN を設定してください',
        setupSwitchDesc:
          '先に 6 桁 PIN を設定してください。保存後、解除方法は PIN に切り替わります',
        disableDesc: '現在のパスワードを入力してプライバシー保護を無効化してください',
        resetDesc: '新しいパスワードを設定する前に現在のパスワードを確認してください',
        windowsEnableTitle: 'プライバシー保護を有効化',
        windowsDisableTitle: 'プライバシー保護を無効化',
        windowsEnableDesc:
          'Windows サインインパスワードを入力してください。認証後にプライバシー保護が有効になります',
        windowsDisableDesc:
          'Windows サインインパスワードを入力してプライバシー保護を無効化してください',
        currentPassword: '現在のパスワード',
        newPassword: '新しいパスワード',
        confirmPassword: '新しいパスワード（確認）',
        windowsPasswordLabel: 'Windows サインインパスワード',
        windowsPasswordPlaceholder: 'Windows サインインパスワードを入力',
        verifyAndEnable: '検証して有効化',
        confirmDisable: '無効化を確認'
      }
    },
    data: {
      card: 'データ管理',
      export: 'データをエクスポート',
      exportDescription:
        'データベースと添付ファイルを ZIP バックアップにエクスポートします。画像ディレクトリはパスワード付き ZIP として保存されます',
      import: 'データをインポート',
      importDescription:
        'ZIP バックアップからデータを復元します。ローカルデータは上書きされ、バックアップパスワードが必要です',
      backupPassword: 'バックアップパスワード',
      confirmPassword: 'パスワード確認',
      minLength: '{count} 文字以上',
      reenterPassword: 'パスワードを再入力',
      exportPasswordTitle: 'バックアップのエクスポートパスワード',
      importPasswordTitle: 'バックアップのインポートパスワード',
      exportPasswordDesc:
        'このバックアップのパスワードを設定してください。インポート時に必要になります',
      importPasswordDesc: 'エクスポート時に設定したバックアップパスワードを入力してください',
      confirmExport: 'エクスポートを確認',
      confirmImport: 'インポートを確認',
      exportInProgress: 'バックアップをエクスポート中',
      importInProgress: 'バックアップをインポート中',
      preparingExport: 'エクスポートタスクを準備中...',
      preparingImport: 'インポートタスクを準備中...',
      exporting: 'データをエクスポート中...',
      importing: 'データをインポート中...',
      cancelingExport: 'エクスポートをキャンセル中...',
      cancelingImport: 'インポートをキャンセル中...',
      exportCanceled: 'エクスポートをキャンセルしました',
      importCanceled: 'インポートをキャンセルしました',
      canceledExportMessage: 'エクスポートをキャンセルしました',
      canceledImportMessage: 'インポートをキャンセルしました',
      exportCompleted: 'エクスポート完了',
      importCompleted: 'インポート完了',
      exportFailed: 'エクスポートに失敗しました',
      importFailed: 'インポートに失敗しました',
      importSuccessAndReload: 'インポート成功。完了を押してページを再読み込みしてください',
      exportSuccessWithPath: 'エクスポートに成功しました: {path}',
      exportAction: 'エクスポート',
      importAction: 'インポート',
      failedWithReason: '{action}に失敗しました: {reason}',
      updateFailedWithReason: '更新に失敗しました: {reason}',
      openFailedWithReason: '有効化に失敗しました: {reason}',
      closeFailedWithReason: '無効化に失敗しました: {reason}',
      saveFailedWithReason: '保存に失敗しました: {reason}',
      checkUpdateFailedWithReason: 'アップデート確認に失敗しました: {reason}',
      downloadUpdateFailedWithReason: 'アップデートのダウンロードに失敗しました: {reason}',
      unknownError: '不明なエラー',
      wrongBackupPassword: 'バックアップパスワードが正しくありません。再試行してください',
      backupPasswordTooShort: 'バックアップパスワードは最低 {count} 文字必要です',
      unsupportedBackup:
        'サポートされていないバックアップ形式です。現在のアプリバージョンで作成されたバックアップのみ対応しています',
      missingEnvelope: 'バックアップにキーファイルがなく、インポートできません',
      transferInProgress: '別のインポート/エクスポートタスクがすでに実行中です',
      importConfirmTitle: 'インポートの確認',
      importConfirmContent:
        'ZIP バックアップをインポートするとローカルデータが上書きされます。先にバックアップのエクスポートを推奨します。続行しますか？',
      continueImport: 'インポートを続行'
    },
    about: {
      card: 'このアプリについて',
      version: 'バージョン',
      source: 'ソース',
      checkUpdate: '新しいバージョンを確認',
      checkUpdateButton: 'アップデート確認',
      downloadUpdate: 'アップデートをダウンロード',
      installNow: '今すぐインストール',
      updateFound: '新しいバージョンが見つかりました: {version}',
      latest: '現在、最新バージョンを使用しています',
      downloading: 'アップデートをダウンロード中...',
      downloadProgressDetail: 'ダウンロード済み {transferred} / 合計 {total}',
      cancelingDownload: 'ダウンロードをキャンセル中...',
      downloadCanceled: 'アップデートのダウンロードをキャンセルしました',
      downloaded:
        'アップデートをダウンロードしました。インストールをクリックして再起動し適用してください'
    }
  }
}

export default jaJP
